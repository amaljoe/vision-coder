...
Standard SFT trains on image-HTML pairs, but the same reference image can be
reproduced by many structurally different HTML strings.  The supervision signal
is therefore ambiguous: visually equivalent outputs differ at the token level,
reducing sample efficiency---the model extracts less visual information per
training example than a reward signal that directly measures rendered output.
...
image), the model generates $G = 8$ candidate HTML outputs (Figure~\ref{fig:arch}).  The advantage for
candidate $i$ is:
\[
...
tokens in the trajectory weighted by $A_i$.  We use the DR-GRPO loss~\citep{vonwerra2022trl}
...
\noindent\textbf{Content Multiplier.}
We observed that blank rendered pages received composite rewards of
$\approx\!0.3$, higher than many low-quality but non-blank pages, because
format, validity, and colour sub-rewards all partially fire on empty white
outputs.  Left unchecked, this would incentivise the model to generate blank
pages.  We address this with a binary \emph{content multiplier} $C$:
\[
  r_{\text{final}} = C \cdot r_{\text{total}}, \quad
  C = \begin{cases} 0 & \text{if non-white pixels} < 0.5\%\\
                    1 & \text{otherwise,} \end{cases}
\]
where ``non-white'' is measured on the predicted render when the reference has
content.  Blank pages receive exactly zero reward and are therefore heavily
penalised relative to any partial attempt.
...
\begin{figure}[t]
  ...
  \caption{Training dynamics over 500 GRPO steps. \textit{Left:} Component
    reward curves (Format, Validity, CLIP, Structural), each in the $[0,1]$
    range. Format and validity stabilise by step~100; CLIP rises steadily
    thereafter. \textit{Right:} Mean completion length drops from
    $\approx$1{,}000 to $\approx$460 tokens as the model learns to write
    concise HTML.}
  \label{fig:training}
\end{figure}
...
metrics.  After step 100, CLIP and structural rewards rise steadily.
Completion length drops sharply from around 1{,}000 tokens to around 460:
the model learns to write compact HTML rather than filling the context with
redundant tags and inline comments.
...
single-column pages) come out very close to the reference.  Medium examples
(multi-section pages) reproduce content and layout well, though gradient-heavy
pages show color inaccuracies and there are some positional offsets.  Hard examples (complex multi-column dashboards) are only
partially reproduced; deeply nested CSS grid and flex structures are where the
model still struggles most.  Hard examples occasionally produce completely blank
pages: the model tends to write CSS first, and on complex layouts the
stylesheet alone consumes the entire token budget, leaving no tokens for the
HTML structure.
